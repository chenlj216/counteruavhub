...
const INPUT_CLS = 'w-full border border-gray-300 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none'
...
export default function JammerCalculatorPage() {
  ...
  const calculations = useMemo(() => {
    ...
    const pEff = jbw > dbw ? jp * (dbw / jbw) : jp
    const gcsAtDroneDbm = calcGCSAtDrone(sp, sg, sd, f)
    const js = calcJS(pEff, jg, jd, sp, sg, sd)
    const maxRange = calcMaxRange(pEff, jg, f, gcsAtDroneDbm)
    const v = verdict(js)
  ...
  }, [jp, jg, jd, sp, sg, sd, jbw, dbw, f, valid])
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
              ...
              <div className="flex flex-wrap gap-2">
                {selectedDrone.controlFreqMHz.map((fq, i) => (
                  <button
                    ...
                    className={`text-xs px-3 py-1 rounded-full font-mono transition-colors ${selectedBandIdx === i
      ...
      {/* Results */}
      {valid && calculations && (
        <div className={`border rounded-xl overflow-hidden mb-6 ${calculations.v.border} ${calculations.v.bg}`}>
          <div className="grid grid-cols-2 divide-x divide-gray-200 p-6">
            <div className="pr-6">
              ...
              <p className={`text-3xl font-bold font-mono ${calculations.v.color}`}>
                {calculations.js >= 0 ? '+' : ''}{calculations.js.toFixed(1)} <span className="text-lg">dB</span>
              </p>
              <p className={`text-sm font-semibold mt-1 ${calculations.v.color}`}>{calculations.v.label}</p>
            </div>
            <div className="pl-6">
              <p className="text-xs text-gray-500 mb-1">Max Jamming Range (J/S = 0 dB)</p>
              <p className="text-3xl font-bold font-mono text-gray-900">{fmt(calculations.maxRange)}</p>
              <p className={`text-sm mt-1 ${jd <= calculations.maxRange ? 'text-green-700' : 'text-red-600'}`}>
                {jd <= calculations.maxRange
                  ? `current position (${fmt(jd)}) is within range ✓`
                  : `move ${fmt(jd - calculations.maxRange)} closer to jam`}
              </p>
            </div>
          </div>
          <div className="px-6 pb-5 border-t border-gray-200 pt-3 text-xs space-y-1.5 text-gray-500">
            <p>
              ...
              {calculations.bwPenaltyDb < 0 && (
                <span className="text-orange-600 font-semibold">
                  {' '}→ effective {(10 * Math.log10(calculations.pEff) + jg + 30).toFixed(1)} dBm after BW penalty
                </span>
              )}
            </p>
            <p>GCS EIRP: {(10 * Math.log10(sp) + sg + 30).toFixed(1)} dBm</p>
            <p>GCS signal at drone: <span className="font-mono">{calculations.gcsAtDroneDbm.toFixed(1)} dBm</span> (Friis, {sd} m)</p>
            <p className={calculations.bwPenaltyDb < 0 ? 'text-orange-600 font-semibold' : ''}>
              Bandwidth penalty: {calculations.bwPenaltyDb.toFixed(1)} dB
              {calculations.bwPenaltyDb < 0
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden mb-8">
          <div className="px-6 py-3 bg-gray-50 border-b border-gray-200">
            <h2 className="font-bold text-gray-900 text-sm">
              ...
              {calculations.bwPenaltyDb < 0 && ` · BW penalty ${calculations.bwPenaltyDb.toFixed(1)} dB`}
            </h2>
          </div>
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {powers.map((pw) => {
                ...
                const rangeRow = calcMaxRange(pwEff, jg, f, calculations.gcsAtDroneDbm)
