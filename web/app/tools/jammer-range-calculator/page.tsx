...
function calcJammerRange(
  ...
  // Effective jammer range: R = (λ/4π) × √(Pj × Gj / P_min)
  // Friis: Pr = Pt × Gt × (λ/4πR)² — solved for R, Gr=1 (drone omni)
    ...
    <main className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 text-sm">
          ...
          disrupt drone communications, based on jammer EIRP, frequency, and target receiver
          sensitivity.
        </p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden mb-6">
        <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
          <h2 className="font-bold text-gray-900 text-sm">Jammer Parameters</h2>
        </div>
        <div className="px-6 py-5 space-y-5">
          ...
          <div>
            ...
            <input
              type="number" min="0.1" step="0.1" value={powerW}
          ...
          <div>
            ...
            <input
              type="number" step="0.5" value={gainDbi}
          ...
          <div>
            ...
            <input
              type="number" min="1" value={freqMHz}
              onChange={(e) => setFreqMHz(e.target.value)}
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-red-400"
            />
            <div className="flex flex-wrap gap-2 mt-2">
              {FREQ_PRESETS.map((p) => (
                <button key={p.value} onClick={() => setFreqMHz(String(p.value))}
          ...
          <div>
            ...
            <input
              type="number" step="1" value={sensitivityDbm}
              onChange={(e) => setSensitivityDbm(e.target.value)}
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-red-400"
            />
            <p className="text-xs text-gray-400 mt-1">
              Typical drone receiver: −80 to −90 dBm · Conservative planning: −75 dBm
            </p>
          </div>
        </div>
      </div>
      ...
      {range !== null && eirpDbm !== null && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-6 mb-6">
          ...
          <p className="text-xs text-gray-500 mt-3">
            Assumes free-space propagation, drone receiver gain = 0 dBi. Real-world range is typically
            30–60% of this value due to multipath and terrain.
          </p>
        </div>
      )}
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden mb-8">
          <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
            <h2 className="font-bold text-gray-900 text-sm">
              Range vs Power at {g} dBi gain · {f >= 1000 ? (f / 1000) + ' GHz' : f + ' MHz'}
            </h2>
          </div>
